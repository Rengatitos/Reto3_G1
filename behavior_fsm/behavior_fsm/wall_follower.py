...
class WallFollower(Node):
    ...
    def __init__(self):
        ...
        # — Parametros ————————————————————————————————————————————————————
        self.declare_parameter('dist_objetivo',  0.50)   # m  — dist. a pared derecha (referencia principal)
        self.declare_parameter('ancho_pasillo',  0.60)   # m  — ancho total del jiron (para fallback a pared izq)
        ...
        self._ancho    = self.get_parameter('ancho_pasillo').value
        ...
        self.get_logger().info(
            ...
            f'  dist_obj={self._d_obj} m  ancho_pasillo={self._ancho} m'
    ...
    def _calcular_error(self, izq, der):
        """
        Error de seguimiento de la pared derecha (referencia fija).

        Siempre se ancla a la pared derecha cuando es visible, ignorando
        la izquierda aunque tambien este detectada (la pista es un
        rectangulo con cajas pegadas a las paredes; se sigue un solo lado).
        Si la derecha desaparece momentaneamente (hueco, esquina), usa la
        izquierda como referencia de respaldo para no perder el control lateral,
        pero el objetivo en la izquierda es (ancho_pasillo - dist_objetivo), NO
        dist_objetivo: dist_objetivo esta calibrado para la pared derecha, y
        usarlo tal cual contra la izquierda arrima el robot a la isla central.

        Solo derecha:  error = dist_objetivo + mean_y_der
        Sin derecha, solo izquierda (respaldo):
            error = mean_y_izq - (ancho_pasillo - dist_objetivo)
        """
        if der:
            return self._d_obj + der['mean_y']
        if izq:
            return izq['mean_y'] - (self._ancho - self._d_obj)
        return None
